...
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one SoloClaw."
...
GITHUB_RELEASE_MIRROR="https://npmmirror.com/mirrors/github-release"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.ps1 | iex"
        exit 1
    fi
...
run_npm_global_install() {
    ...
    local -a cmd
    cmd=(env "SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS" npm --loglevel "$NPM_LOGLEVEL" --registry "$NPM_REGISTRY")
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash -s -- [options]
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash"
}
...
install_node() {
    if [[ "$OS" == "macos" ]]; then
        ...
        if command -v apt-get &> /dev/null; then
            ...
            tmp="$(mktempfile)"
            # 使用国内镜像源
            download_file "https://mirrors.tuna.tsinghua.edu.cn/nodesource/deb/setup_${NODE_DEFAULT_MAJOR}.x" "$tmp"
            if is_root; then
            ...
            tmp="$(mktempfile)"
            # 使用国内镜像源
            download_file "https://mirrors.tuna.tsinghua.edu.cn/nodesource/rpm/setup_${NODE_DEFAULT_MAJOR}.x" "$tmp"
            if is_root; then
                run_quiet_step "Configuring NodeSource repository" bash "$tmp"
...
main() {
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/yaowenjie0805-sys/openclaw/main/scripts/install.sh | bash -s -- --install-method npm"
